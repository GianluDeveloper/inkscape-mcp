...
export function Dashboard() {
  ...
  return (
    <div className="space-y-6" data-testid="dashboard">
      {/* Hero */}
      <Card
        className="border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900/50 to-blue-950/20"
        data-testid="hero"
      >
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <span className="rounded-md bg-slate-800 px-2 py-0.5 text-sm text-slate-300 font-mono">
                  v{h?.version ?? "—"}
                </span>
              </div>
              <p className="max-w-2xl text-sm text-slate-200">
                {h?.description ??
                  "AI-powered vector graphics server. Load the backend to see status."}
              </p>
              <div className="flex flex-wrap gap-4 pt-1 text-sm text-slate-400">
                ...
                {h?.uptime_seconds != null && (
                  <span>
                    Up {Math.floor(h.uptime_seconds / 60)}m{" "}
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              >
                <span
                  className={`relative flex h-2.5 w-2.5 ${err ? "bg-red-500" : h ? "bg-emerald-500" : "bg-gray-500"} rounded-full`}
                >
                  {!err && h && (
                ...
                <span
                  className={`${err ? "text-amber-400" : h ? "text-emerald-400" : "text-gray-400"}`}
                >
      ...
      {err && (
        <div
          className="rounded-lg border border-red-800 bg-red-950/20 px-4 py-3 text-sm text-red-400"
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        >
          ...
          <CardContent>
            ...
            <p className="text-sm text-slate-300">{h?.version ?? ""}</p>
          </CardContent>
        </Card>
        ...
        >
          ...
          <CardContent>
            ...
            <p className="text-sm text-slate-300">
              {h?.tool_groups?.length ?? 0} portmanteau groups
            </p>
          </CardContent>
        </Card>
        ...
        >
          ...
          <CardContent>
            ...
            <p className="text-sm text-slate-300">
              {h?.providers?.ollama?.model ?? "SVG generation only"}
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            {h.tool_groups.map((g) => {
              ...
              return (
                <Link
                  key={g.name}
                  to={
                    g.category === "layer_management"
                      ? "/layers"
                      : g.category === "animation"
                        ? "/animation"
                        : "/agent-tools"
                  }
          ...
          <CardContent>
            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-2 text-sm">
                <span
                  className={`h-2 w-2 rounded-full ${inkOk ? "bg-emerald-500" : "bg-slate-600"}`}
                />
                <span className="text-slate-300">Inkscape</span>
                <span className="text-slate-600">
                  {inkOk
                    ? (h.providers.inkscape?.version_line ?? "found")
                    : "not installed"}
                </span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span
                  className={`h-2 w-2 rounded-full ${ollOk ? "bg-emerald-500" : "bg-slate-600"}`}
                />
                <span className="text-slate-300">Ollama</span>
                <span className="text-slate-600">
                  {ollOk ? h.providers.ollama?.base_url : "unreachable"}
                </span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span
                  className={`h-2 w-2 rounded-full ${h.providers.gemini_key ? "bg-emerald-500" : "bg-slate-600"}`}
                />
                <span className="text-slate-300">Gemini</span>
                <span className="text-slate-600">
                  {h.providers.gemini_key ? "key set" : "no key"}
                </span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span
                  className={`h-2 w-2 rounded-full ${h.providers.anthropic_key ? "bg-emerald-500" : "bg-slate-600"}`}
                />
                <span className="text-slate-300">Anthropic</span>
                <span className="text-slate-600">
                  {h.providers.anthropic_key ? "key set" : "no key"}
                </span>
              </div>
            </div>
          </CardContent>
      ...
      <div className="flex flex-wrap gap-3 text-sm">
        <Link to="/status" className="text-blue-400 hover:underline">
          Full status JSON &rarr;
        </Link>
        <Link to="/agent-tools" className="text-blue-400 hover:underline">
          Agent Lab &rarr;
        </Link>
        <Link to="/animation" className="text-blue-400 hover:underline">
          Animation Studio &rarr;
        </Link>
        <Link to="/layers" className="text-blue-400 hover:underline">
          Layer Manager &rarr;
        </Link>
