  ...
  const openApp = async (app: FleetApp) => {
    ...
    try {
      ...
      if (
        ...
        j.status === "brought_to_foreground"
      ) {
        if (j.status === "brought_to_foreground") {
          if (!j.alive) {
            setError(
              `${app.id}: Tauri window brought to front — no web UI on :${app.port}`,
            );
          } else {
            setTimeout(
              () =>
                window.open(
                  j.url ?? app.url ?? `http://127.0.0.1:${app.port}`,
                  "_blank",
                ),
              300,
            );
        ...
        for (let i = 0; i < 15; i++) {
          ...
          if (hr) {
            ...
            if (hr.alive) {
              window.open(
                hr.health_url ?? app.url ?? `http://127.0.0.1:${app.port}`,
                "_blank",
              );
              break;
            }
        ...
        const headless =
          j.error && /no.*start|headless|no web/i.test(String(j.error));
    ...
    <div className="space-y-6" data-testid="apps-page">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            <h1 className="text-2xl font-bold tracking-tight">
              Fleet apps — start or switch
            </h1>
            <p className="text-sm text-muted-foreground leading-relaxed max-w-[70ch]">
              All installed{" "}
              <span className="font-mono text-xs">sandraschi</span> repos with a
              webapp (from{" "}
              <span className="font-mono text-xs">fleet-registry.json</span> +{" "}
              <span className="font-mono text-xs">WEBAPP_PORTS.md</span>). Click
              a card to check{" "}
              <span className="font-mono text-xs">:{`{port}`}/health</span>{" "}
              first. If it is up, we open it; if a Tauri winapp is already
              running we bring it to the foreground; otherwise we start it via{" "}
              <span className="font-mono text-xs">
                mcd starts\{`{id}`}-start.bat
              </span>{" "}
              → <span className="font-mono text-xs">repo\start.ps1</span> →
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      ...
                      {app.has_tauri && (
                        <span
                          ...
                          title={
                            app.has_tauri_installed
                              ? "Tauri winapp installed"
                              : "Tauri capable"
                          }
                        >
            ...
            {filtered.map((app) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs font-mono text-gh-green">
                    :{app.port}
                  </span>
                  <span className="text-xs font-mono">{app.category}</span>
                  <span className="text-xs font-mono text-muted-foreground">
                    {app.last_commit
                      ? new Date(app.last_commit).toLocaleDateString()
                      : "—"}
                  </span>
      ...
      <div className="rounded-lg border border-dashed border-border bg-black/20 p-4 flex items-start gap-3">
        ...
        <div className="text-xs text-muted-foreground leading-relaxed">
          <span className="font-semibold text-foreground">Fleet standard:</span>{" "}
          every <span className="font-mono">Apps</span> page checks{" "}
          <span className="font-mono">:{`{port}`}/health</span> first; if a
          Tauri winapp is already running we bring it to front, otherwise we
          launch{" "}
          <span className="font-mono">mcd starts\{`{id}`}-start.bat</span> →{" "}
          <span className="font-mono">repo\start.ps1</span> → Tauri exe and then
          open. Descriptions prefer{" "}
          <span className="font-mono">pyproject.toml description</span> (plain)
          with <span className="font-mono">gh repo</span> fallback — optimize
          there.
        </div>
      </div>
    </div>
