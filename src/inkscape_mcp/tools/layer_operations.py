...
_LAYER_RE = re.compile(r'(<g\b[^>]*inkscape:groupmode="layer"[^>]*>.*?</g>)', re.DOTALL)
...
async def inkscape_layers(
    operation: Literal[
        "list", "get", "create", "rename", "hide", "show", "reorder", "lock", "unlock"
    ],
    input_path: str = "",
    output_path: str = "",
    layer_id: str = "",
    ...
    try:
        if operation == "list":
            ...
            return ok(
                "list", f"Found {len(layers)} layer(s)", {"layers": layers, "count": len(layers)}
            )
            ...
            if not match:
                return fail(
                    ...
                    data={"available": [ly["id"] for ly in layers]},
                )
            ...
            return ok(
                "create", f"Created layer '{lid}' ({lbl})", {"id": lid, "label": lbl, "path": dest}
            )
            ...
            return ok(
                "rename",
                f"Renamed '{layer_id}' → '{new_label}'",
                {"id": layer_id, "label": new_label},
            )
            ...
            if position < 0 or position >= len(layers):
                return fail(
                    "reorder",
                    f"Position {position} out of range (0-{len(layers) - 1})",
                    "ValueError",
                )
            ...
            return ok(
                "reorder",
                f"Moved '{layer_id}' to position {position}",
                {"id": layer_id, "position": position},
            )
