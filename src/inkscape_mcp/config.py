...
class InkscapeConfig(BaseModel):
    ...
    process_timeout: int = Field(
        default=30, ge=5, le=300, description="Timeout for Inkscape operations in seconds"
    )
    ...
    preserve_metadata: bool = Field(
        default=True, description="Preserve EXIF and other metadata when possible"
    )
    ...
    default_quality: int = Field(
        default=95, ge=1, le=100, description="Default JPEG quality (1-100)"
    )
    ...
    enable_batch_operations: bool = Field(
        default=True, description="Enable batch processing capabilities"
    )
    ...
    batch_size_limit: int = Field(
        default=50, ge=1, le=200, description="Maximum number of files in a single batch"
    )
    ...
    enable_performance_logging: bool = Field(
        default=False, description="Enable detailed performance logging"
    )
    ...
    def load_default(cls) -> "InkscapeConfig":
        ...
        config_paths = [
            ...
            Path.cwd() / "inkscape-mcp.yaml",
            Path.home() / ".inkscape-mcp" / "config.yaml",
            Path.home() / ".config" / "inkscape-mcp" / "config.yaml",
        ]
    ...
    def _detect_inkscape_executable() -> str | None:
        ...
        candidate = os.environ.get("INKSCAPE_PATH") or shutil.which("inkscape") or shutil.which("inkscape.exe")
...
def load_config(config_path: str | Path | None = None) -> InkscapeConfig:
    ...
    if config_path is None:
        ...
        config_dir = Path.home() / ".config" / "inkscape-mcp"
    ...
    try:
        ...
        return InkscapeConfig.load_from_file(config_path)
    except Exception as e:
