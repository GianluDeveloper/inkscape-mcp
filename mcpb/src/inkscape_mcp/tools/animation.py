...
async def inkscape_animation(
    ...
    def fail(
        op: str, msg: str, err: str = "", data: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        return AnimResult(
            success=False,
            operation=op,
    ...
    if operation == "list_presets":
        return ok(
            "list_presets",
            f"{len(_PRESETS)} animation presets available",
    ...
    if operation == "apply_preset":
        ...
        try:
            ...
            return ok(
                "apply_preset",
                f"Generated '{preset_name}' animation at {output_path}",
        ...
        if input_path:
            ...
            try:
                ...
                if not pattern.search(svg):
                    return fail(
                        "animate_element", f"Element '{target_id}' not found in {input_path}"
                    )
                new_svg = pattern.sub(_inject, svg)
                Path(output_path).write_text(new_svg, encoding="utf-8")
                return ok(
                    "animate_element",
                    f"Animation injected into '{target_id}'",
                    {
                        "target_id": target_id,
                        "attribute": attribute,
                        "values": values,
            ...
            return ok(
                "animate_element",
                f"Created animated SVG at {output_path}",
                {
                    "target_id": target_id,
                    "attribute": attribute,
                    "values": values,
    ...
    if operation == "animate_transform":
        ...
        if "translate" in attribute.lower() or attribute in ("x", "y"):
            ...
            anim_type = "scale"
        anim = (
            f'<animateTransform attributeName="transform" type="{anim_type}" '
        ...
        return ok(
            "animate_transform",
            f"Transform animation for '{target_id}'",
            {
                "target_id": target_id,
                "type": anim_type,
                "values": values,
    ...
    if operation == "animate_motion":
        ...
        d = (
            values
            or f"M {x - width / 2},{y} C {x - width / 4},{y - height / 4} {x + width / 4},{y + height / 4} {x + width / 2},{y}"
        )
        ...
        return ok(
            "animate_motion",
            f"Motion path animation for '{target_id}'",
    ...
    if operation == "animate_color":
        ...
        return ok(
            "animate_color",
            f"Color animation for '{target_id}'",
            {
                "target_id": target_id,
                "attr": attr,
                "from": color_from or fill,
                "to": color_to,
                "output_path": output_path,
            },
        )
...
.{animation_name} {{
  animation: {animation_name} {duration}s {repeat.replace("indefinite", "infinite")};
        ...
        return ok(
            "css_animation",
            f"CSS animation '{animation_name}' at {output_path}",
            {
